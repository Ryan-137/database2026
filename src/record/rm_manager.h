...
class RmManager {
   ...
   public:
    ...
    void create_file(const std::string& filename, int record_size) {
        ...
        // 记录页真实布局：页内 LSN 偏移 + RmPageHdr + bitmap + slots，按该布局计算最大槽位数
        int page_capacity = PAGE_SIZE - Page::OFFSET_PAGE_HDR - static_cast<int>(sizeof(RmPageHdr));
        file_hdr.num_records_per_page =
            (BITMAP_WIDTH * (page_capacity - 1) + 1) / (1 + record_size * BITMAP_WIDTH);
    ...
    void close_file(const RmFileHandle* file_handle) {
        ...
        if (!buffer_pool_manager_->discard_all_pages(file_handle->fd_)) {
            throw InternalError("Pinned heap page while closing file");
        }
