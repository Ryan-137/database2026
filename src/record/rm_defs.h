...
struct RmRecord {
    ...
    RmRecord(const RmRecord &other) { CopyFrom(other); }
    ...
    RmRecord(RmRecord &&other) noexcept { MoveFrom(std::move(other)); }
    ...
    explicit RmRecord(int size_) : data(size_ > 0 ? new char[size_] : nullptr), size(size_), allocated_(size_ > 0) {}

    RmRecord(int size_, const char *data_)
        : data(size_ > 0 ? new char[size_] : nullptr), size(size_), allocated_(size_ > 0) {
        if (size_ > 0) {
    ...
    void Deserialize(const char* data_) {
        int new_size = *reinterpret_cast<const int*>(data_);
        char *new_data = new_size > 0 ? new char[new_size] : nullptr;
        if (new_size > 0) {
            memcpy(new_data, data_ + sizeof(int), new_size);
        }
        Release();
        data = new_data;
        size = new_size;
        allocated_ = new_size > 0;
    }

    ~RmRecord() { Release(); }

   private:
    void CopyFrom(const RmRecord &other) {
        size = other.size;
        if (size > 0) {
            data = new char[size];
    ...
    void MoveFrom(RmRecord &&other) noexcept {
        data = other.data;
        size = other.size;
