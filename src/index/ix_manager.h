...
class IxManager {
   ...
   public:
    ...
    void close_index(const IxIndexHandle *ih) {
        // resident 根页必须先恢复为普通可淘汰页，随后 flush_all_pages 才能
        // 完成最终写回并在关闭 fd 前清理持久状态。
        ih->release_resident_root();
        ...
        if (!buffer_pool_manager_->discard_all_pages(ih->fd_)) {
            throw InternalError("Pinned index page while closing file");
        }
