...
class Optimizer {
   ...
   public:
    ...
    std::shared_ptr<Plan> plan_query(std::shared_ptr<Query> query, Context *context) {
        ...
        } else if (auto x = std::dynamic_pointer_cast<ast::SetTransactionIsolationStmt>(query->parse)) {
            auto isolation = x->isolation == ast::TxnIsolationLevel::SNAPSHOT_ISOLATION
                                 ? IsolationLevel::SNAPSHOT_ISOLATION
                                 : IsolationLevel::SERIALIZABLE;
            return std::make_shared<SetTransactionIsolationPlan>(isolation);
        } else {
