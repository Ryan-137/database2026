...
class Planner {
   ...
   private:
    ...
    std::shared_ptr<Plan> make_left_deep_join_plan(const std::shared_ptr<Query> &query, Context *context);
    ...
    bool get_skip_scan_index_cols(const std::string &tab_name, const std::vector<Condition> &curr_conds,
                                  std::vector<std::string> &index_col_names,
                                  const std::string &visible_tab_name = "");
    bool is_unique_point_predicate(const std::string &tab_name, const std::vector<Condition> &conds);
