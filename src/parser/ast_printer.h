...
private:
    ...
    static void print_node(const std::shared_ptr<TreeNode> &node, int offset) {
        ...
        if (auto x = std::dynamic_pointer_cast<Help>(node)) {
        ...
        } else if (auto x = std::dynamic_pointer_cast<UpdateExpr>(node)) {
            std::cout << "UPDATE_EXPR\n";
            if (x->kind == UpdateExprKind::VALUE) {
                print_node(x->value, offset);
            } else if (x->kind == UpdateExprKind::COLUMN) {
                print_node(x->column, offset);
            } else {
                print_val(x->kind == UpdateExprKind::ADD ? "+" : "-", offset);
            ...
            std::cout << "ROLLBACK\n";
        } else if (auto x = std::dynamic_pointer_cast<SetTransactionIsolationStmt>(node)) {
            std::cout << "SET_TRANSACTION_ISOLATION\n";
            print_val(x->isolation == TxnIsolationLevel::SNAPSHOT_ISOLATION ? "SNAPSHOT ISOLATION" : "SERIALIZABLE",
                      offset);
        } else {
            assert(0);
        }
